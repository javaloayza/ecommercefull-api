...
const DB_URL = process.env.DATABASE_URL || `postgres://${DB_USER}:${DB_PASSWORD}@${DB_HOST}:5432/ecommercefull_n7wy`
...
/* Calling the function that is exported from each model file. */
modelDefiners.forEach(model => model(sequelize))
const entries = Object.entries(sequelize.models)
/* Capitalizing the first letter of the model names. */
const capsEntries = entries.map((entry) => [entry[0][0].toUpperCase() + entry[0].slice(1), entry[1]])
sequelize.models = Object.fromEntries(capsEntries)

/* Destructuring the models from the sequelize object. */
