...
async function getCartItems (userId) {
  try {
    ...
    let cartItems = await Product.findAll({
      ...
      //* Telling Sequelize to return the data in a format that is easier to work with. */
      raw: false
    })
    cartItems = cartItems.map(item => {
      cart.forEach(product => {
        if (item.dataValues.id === product.productId) {
          item.dataValues.quantity = product.quantity
        }
      })
      return item
    })
